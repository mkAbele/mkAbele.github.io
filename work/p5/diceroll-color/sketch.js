...
function findPolygons() {
  // A simple way to find polygons is to look for closed loops of lines.
  // This can be done by tracking visited vertices and edges.
  let vertices = {};

  // Collect vertices and edges
  lines.forEach(line => {
    let v1 = `${line.x1},${line.y1}`;
    let v2 = `${line.x2},${line.y2}`;
    if (!vertices[v1]) vertices[v1] = [];
    if (!vertices[v2]) vertices[v2] = [];
    vertices[v1].push(v2);
    vertices[v2].push(v1);
  });

  // Function to find a polygon starting from an edge
  function findPolygon(start, visitedEdges) {
    let polygon = [];
    let current = start;
    while (true) {
      let [currentPoint, nextPoint] = current.split('->');
      polygon.push(currentPoint.split(',').map(Number));
      visitedEdges.add(current);
      let nextEdges = vertices[nextPoint].map(v => `${nextPoint}->${v}`);
      let nextEdge = nextEdges.find(e => !visitedEdges.has(e));
      if (!nextEdge) break; // No more edges to follow
      current = nextEdge;
    }
    return polygon;
  }
  ...
  Object.keys(vertices).forEach(vertex => {
    vertices[vertex].forEach(nextVertex => {
      let edge = `${vertex}->${nextVertex}`;
      if (!visitedEdges.has(edge)) {
        let polygon = findPolygon(edge, visitedEdges);
        if (polygon.length > 2) { // At least a triangle
...
function drawPolygons() {
  polygons.forEach(polygon => {
    ...
    beginShape();
    polygon.forEach(([x, y]) => vertex(x, y));
    endShape(CLOSE);
  });
}